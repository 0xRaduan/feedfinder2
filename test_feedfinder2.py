...
class TestFeedGuessing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.html_content = "<html><head><title>Test Page</title></head><body></body></html>"
    ...
    def mock_requests_get_logic(self, url, headers=None, timeout=None):
        ...
        if url in self.mock_responses:
            # print(f"Mocking URL: {url} with content: {self.mock_responses[url][:30]}...")
    ...
    def test_sort_order_of_guessed_feeds(self, mock_get):
        ...
        # From leaf_filenames, "feed" has higher priority than "atom" based on url_feed_prob
    ...
    # Test for feeds found via <link> tags - ensure guessing is skipped if check_all=False
    @patch('feedfinder2.requests.get')
    def test_feeds_from_link_tags_skips_guessing_if_not_check_all(self, mock_get):
        ...
        self.mock_responses[guessed_feed_url] = self.feed_content # Make this a valid feed too

        feeds = find_feeds(test_url, check_all=False) # check_all=False
        self.assertIn(linked_feed_url, feeds)
        self.assertNotIn(guessed_feed_url, feeds, "Guessing should not have occurred if a feed was found via <link> and check_all=False")
        self.assertEqual(len(feeds), 1)
    ...
    def test_feeds_from_link_tags_does_guessing_if_check_all_true(self, mock_get):
        ...
        self.assertIn(linked_feed_url, feeds)
        self.assertIn(guessed_feed_url, feeds, "Guessing should have occurred with check_all=True")
